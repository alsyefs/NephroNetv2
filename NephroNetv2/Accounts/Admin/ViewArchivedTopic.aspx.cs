...
{
    public partial class ViewArchivedTopic : System.Web.UI.Page
    {
        static string conn = "";
        ...
        protected string getHeader()
        {
            ...
            if (countTopic > 0)//if ID exists, countTopic = 1
            {
                ...
                cmd.CommandText = "select userId from Users where loginId = '" + loginId + "' ";
                string userId = cmd.ExecuteScalar().ToString();
                header = Layouts.postHeaderArchive(creator, topic_type, topic_title, topic_time, topic_description, imagesHTML, roleId, userId, topicId, topic_createdBy);
            }
            else
            {
                addSession();
        ...
        protected string getContents(int pageNum)
        {
            ...
            connect.Open();
            SqlCommand cmd = connect.CreateCommand();
            //Count entries for this topic that are approved, not denied:
            cmd.CommandText = "select count(*) from Entries where topicId = '" + topicId + "' and entry_isApproved = 1 and entry_isDenied = 0 ";
            int totalEntries = Convert.ToInt32(cmd.ExecuteScalar());
            g_entries = totalEntries;
            for (int i = 1; i <= totalEntries; i++)
            {
                //Get entry ID:
                cmd.CommandText = "select [entryId] from (SELECT rowNum = ROW_NUMBER() OVER(ORDER BY entryId ASC), * FROM [Entries] where topicId = '" + topicId + "' and entry_isApproved = 1 and entry_isDenied = 0 ) as t where rowNum = '" + i + "'";
                string entryId = cmd.ExecuteScalar().ToString();
                //Get entry text:
                cmd.CommandText = "select [entry_text] from (SELECT rowNum = ROW_NUMBER() OVER(ORDER BY entryId ASC), * FROM [Entries] where topicId = '" + topicId + "' and entry_isApproved = 1 and entry_isDenied = 0) as t where rowNum = '" + i + "'";
                string entry_text = cmd.ExecuteScalar().ToString();
                //Get entry time:
                cmd.CommandText = "select [entry_time] from (SELECT rowNum = ROW_NUMBER() OVER(ORDER BY entryId ASC), * FROM [Entries] where topicId = '" + topicId + "' and entry_isApproved = 1 and entry_isDenied = 0 ) as t where rowNum = '" + i + "'";
                string entry_time = cmd.ExecuteScalar().ToString();
                //Get entry's creator:
                cmd.CommandText = "select [userId] from (SELECT rowNum = ROW_NUMBER() OVER(ORDER BY entryId ASC), * FROM [Entries] where topicId = '" + topicId + "' and entry_isApproved = 1 and entry_isDenied = 0 ) as t where rowNum = '" + i + "'";
                ...
                string imagesHtml = "";
                cmd.CommandText = "select [entry_hasImage] from (SELECT rowNum = ROW_NUMBER() OVER(ORDER BY entryId ASC), * FROM [Entries] where topicId = '" + topicId + "' and entry_isApproved = 1 and entry_isDenied = 0 ) as t where rowNum = '" + i + "'";
                ...
                content = content + Layouts.postMessageArchive(i, creator_name, entry_time, entry_text, imagesHtml, entry_creatorId, topic_creatorId, userId, entryId, roleId, topicId);
            }
            connect.Close();
            return content;
        }
        ...
        protected Boolean checkInput()
        {
            ...
            if (string.IsNullOrWhiteSpace(txtEntry.Text))
            {
                ...
                lblEntryError.Text = "Invalid input: Please type something for the message.";
            }
            return correct;
        }
        ...
        [WebMethod]
        [ScriptMethod()]
        public static void retrieveTopic_Click(string topicId, string entry_creatorId)
        {
            requestedRemoveTopic = true;
            Configuration config = new Configuration();
            SqlConnection connect = new SqlConnection(config.getConnectionString());
            bool topicIdExists = isTopicCorrectToRetireve(topicId, entry_creatorId);
            if (topicIdExists)
            {
                connect.Open();
                SqlCommand cmd = connect.CreateCommand();
                //update the DB and set isDeleted = false:
                cmd.CommandText = "update Topics set topic_isDeleted = 0 where topicId = '" + topicId + "' ";
                cmd.ExecuteScalar();
                //Email the topic creator about the topic being deleted:
                cmd.CommandText = "select topic_createdBy from Topics where topicId = '" + topicId + "' ";
                string creatorId = cmd.ExecuteScalar().ToString();
                ...
                connect.Close();
                string emailBody = "Hello " + name + ",\n\n" +
                    "This email is to inform you that your archived topic with the title (" + topic_title + ") has been retrieved. If you think this happened by mistake, or you did not request this action, plaese contact the support.\n\n" +
                    "Best regards,\nNephroNet Support\nNephroNet2018@gmail.com";
                Email email = new Email();
                email.sendEmail(emailTo, emailBody);
            }
        }
        protected static bool isTopicCorrectToRetireve(string topicId, string creatorId)
        {
            bool correct = true;
            CheckErrors errors = new CheckErrors();
            //check if id contains a special character:
        ...
        [WebMethod]
        [ScriptMethod()]
        public static void retrieveMessage_Click(string entryId, string entry_creatorId)
        {
            requestedRemoveMessage = true;
            bool messageIdExists = isMessageCorrectToRetrieve(entryId, entry_creatorId);
            if (messageIdExists)
            {
                Configuration config = new Configuration();
                SqlConnection connect = new SqlConnection(config.getConnectionString());
                ...
                SqlCommand cmd = connect.CreateCommand();
                //update the DB and set isDeleted = false:
                cmd.CommandText = "update Entries set entry_isDeleted = 0 where entryId = '" + entryId + "' ";
                cmd.ExecuteScalar();
                //connect.Close();
                //Email the topic creator about the topic being deleted:
                cmd.CommandText = "select userId from Entries where entryId = '" + entryId + "' ";
                ...
                connect.Close();
                string emailBody = "Hello " + name + ",\n\n" +
                    "This email is to inform you that your message in the topic with the title (" + topic_title + ") has been retireve. The message was:\n" +
                    "\"" + entry_text + "\" \nIf you think this happened by mistake, or you did not request this action, plaese contact the support.\n\n" +
                    "Best regards,\nNephroNet Support\nNephroNet2018@gmail.com";
                Email email = new Email();
                email.sendEmail(emailTo, emailBody);
            }
        }
        protected static bool isMessageCorrectToRetrieve(string messageId, string creatorId)
        {
            Configuration config = new Configuration();
            SqlConnection connect = new SqlConnection(config.getConnectionString());
            bool correct = true;
